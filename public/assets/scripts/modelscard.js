    ...
    fetch('api_addRate', myInit)
        .then((response) => {
            if (response.ok) {
                ...
                //Une erreur réseau c'est produite (voir pour l'afficher)
                return response;
            }
        })
        .then((json) => {
            try {
                if (json.result) {
                    ...
                    const response = json.result.userRate;
                    const globalRate=json.result.rate;
                    const name = `rs-${id}-${response}`;
                    document.getElementById(name).checked = true;
                    const rateName=`global-${id}`;
                    document.getElementById(rateName).textContent=globalRate;
                }
