...
<div class="params-container">
                            ...
                            <div class="user-buttons">
                                ...
                                class="cart-add-card cards-btn"
                                 data-id="{$model->getId()}">
                                    ...
                                    stroke-width="0" 
                                    viewBox="0 0 24 24" 
                                    class="add-cart-model" 
                                    height="1em" 
                                    width="1em" 
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path fill="none" d="M0 0h24v24H0V0z"></path>
                                    <path d="M11 9h2V6h3V4h-3V1h-2v3H8v2h3v3zm-4 9c-1.1 0-1.99.9-1.99 2S5.9 22 7 22s2-.9 2-2-.9-2-2-2zm10 0c-1.1 0-1.99.9-1.99 2s.89 2 1.99 2 2-.9 2-2-.9-2-2-2zm-8.9-5h7.45c.75 0 1.41-.41 1.75-1.03l3.86-7.01L19.42 4l-3.87 7H8.53L4.27 2H1v2h2l3.6 7.59-1.35 2.44C4.52 15.37 5.48 17 7 17h12v-2H7l1.1-2z"></path>
                                    </svg>
                                </button>
                            </div>
                            <div class="card-btn-container">
                                <button 
                                class="update-card cards-btn" 
                                data-id="{$model->getId()}">
                                    <svg 
                                        stroke="currentColor" 
                                        fill="none" 
    ...
    {if isset($connected)}
        ...
        <section class="model-add-form">
            ...
            <form action="parametres_models" class="form-add-model" method="post" id="form-add">
               <div class="form-add-model-inputs-container">
                    ...
                    <label for="new-brand">Marque du kit :
                        {if isset($brands)}
                        <select name="new-brand" id="new-brand" class="select-filter">
                            <option value="0">--</option>
                                {foreach from=$brands item=brand}
                                    <option value="{$brand->getId()}">{$brand->getName()}</option>
                                {/foreach}
                    ...
                    <label for="new-builder">Constructeur :
                        {if isset($builders)}
                        <select name="new-builder" id="new-builder" class="select-filter">
                            <option value="0">--</option>
                                {foreach from=$builders item=builder}
                                    <option value="{$builder->getId()}">{$builder->getName()}</option>
                                {/foreach}
                ...
                <div class="form-add-model-inputs-container">
                    <label for="new-scale">Echelle :
                        {if isset($scales)}
                            <select name="new-scale" id="new-scale" class="select-filter">
                                <option value="0">--</option>
                                {foreach from=$scales item=scale}
                                    <option value="{$scale->getId()}">{$scale->getName()}</option>
                                {/foreach}
                    ...
                    <label for="new-category">Catégorie :
                        {if isset($categories)}
                        <select name="new-category" id="new-category" class="select-filter">
                            <option value="0">--</option>
                            {foreach from=$categories item=category}
                                <option value="{$category->getId()}">{$category->getName()}</option>
                            {/foreach}
                    ...
                    <label for="new-period">Période :
                        {if isset($periods)}
                        <select name="new-period" id="new-period" class="select-filter">
                            <option value="0">--</option>
                                {foreach from=$periods item=period}
                                    <option value="{$period->getId()}">{$period->getName()}</option>
                                {/foreach}
                ...
                <div class="form-add-model-bottom">
                    <label for="new-picture">Photo</label>
                    <input type="file" name="new-picture" id="new-picture" class="">
